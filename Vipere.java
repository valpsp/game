...
/**
 * Classe Vipere a partir de laquelle on va pouvoir instancier des unites Vipere
 * herite de reptile
 * @author Valentin
 *
 */
public class Vipere extends Reptile
{
	...
	private int forceAttaque = 10;
	private int attaquerDeNouveau;
	private int frequenceAttaque = 50; //au plus il est petit au plus la frequence est haute
	 ...
	 */
	public Vipere(Map map, Ressources ressources, Faction factionEnnemie1, Faction factionEnnemie2)
	{
		this.x = 200;
		this.y = 200;
		this.direction = 2;
	 ...
	 * @param map
	 */
	public Vipere(Map map, Ressources ressources, Faction factionEnnemie1, Faction factionEnnemie2, float x, float y)
	{
		this.x = x;
		this.y = y;
		this.direction = 2;
	...
	public void init()
	{
		...
		try
		{
			spriteSheet = new SpriteSheet("src/main/ressources/sprites/monsters/snake.png", 36, 36);
			animations[0] = createAnimation(spriteSheet, 0, 2, 3);
			animations[1] = createAnimation(spriteSheet, 0, 2, 1);
			animations[2] = createAnimation(spriteSheet, 0, 2, 0);
			animations[3] = createAnimation(spriteSheet, 0, 2, 2);
			...
			//Oeuf
			animations[8] = createAnimation(spriteSheet, 9, 11, 7);
			animations[9] = createAnimation(spriteSheet, 9, 11, 5);
			animations[10] = createAnimation(spriteSheet, 9, 11, 4);
			animations[11] = createAnimation(spriteSheet, 9, 11, 6);
			animations[12] = createAnimation(spriteSheet, 9, 11, 7);
			animations[13] = createAnimation(spriteSheet, 9, 11, 5);
			animations[14] = createAnimation(spriteSheet, 9, 11, 4);
			animations[15] = createAnimation(spriteSheet, 9, 11, 6);
			
			//Attaquée
			animations[16] = createAnimation(spriteSheet, 6, 8, 7);
			animations[17] = createAnimation(spriteSheet, 6, 8, 5);
			animations[18] = createAnimation(spriteSheet, 6, 8, 4);
			animations[19] = createAnimation(spriteSheet, 6, 8, 6);
			animations[20] = createAnimation(spriteSheet, 6, 8, 7);
			animations[21] = createAnimation(spriteSheet, 6, 8, 5);
			animations[22] = createAnimation(spriteSheet, 6, 8, 4);
			animations[23] = createAnimation(spriteSheet, 6, 8, 6);
	...
	/**
	 * Affiche d'abord un ombre pour la vipere, puis la vipere par dessus
	 */
	public void render(Graphics g)
	{
		...
		//La vipere se déplace ? On affiche les animations de mouvemnent
		else
		{
			if(attaqueSubie > 10)
				g.drawAnimation(animations[direction+16], x-19, y-33);
	...
	/**
	 * Update l'état de la vipere
	 * On appelle sa fonction d'IA qui gère son comportement
	 */
	public void update(int delta)
	{
		//Pour les tests, où regarde la vipere
		this.regardX = this.x;
		this.regardY = this.y;
	...
	{
		Random rand = new Random();
		int randomPonte = rand.nextInt(500);
	...
	/**
	 * Déplacement manuel de la vipere
	 * @param key
	 */
